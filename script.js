...
while (true) {
  ...
  if (!firstName) break; // Exit loop if the user cancels or leaves input blank
  const lastName = prompt('Enter employee\'s last name:');
  if (!lastName) break; // Exit loop if the user cancels or leaves input blank
  const salary = parseFloat(prompt('Enter employee\'s salary:'));
  if (isNaN(salary)) alert("please enter a number"); // ask for a number if user inputs letters
  else 
